...
def test_required_model_artefacts_exist_and_load():
    ...
    assert hasattr(loaded["model"], "predict_proba"), (
        "Saved model must expose predict_proba"
    )
    assert hasattr(loaded["scaler"], "transform"), (
        "Saved scaler must expose transform"
    )

    feature_names = list(loaded["feature_names"])
    threshold = float(loaded["threshold"])

    assert feature_names, "Saved feature list is empty"
    assert len(feature_names) == len(set(feature_names)), (
        "Saved feature list contains duplicate names"
    )
    assert len(feature_names) == 95, (
        f"Expected 95 saved features, found {len(feature_names)}"
    )
    assert 0 <= threshold <= 1, (
        f"Saved threshold must be between 0 and 1, found {threshold}"
    )
